...
export function createBrowserHistory(
  ...
  // 监听popstate事件，在浏览器返回、前进按钮，window.history.go(),window.history.forward()时触发
  ...
  // index不存在，表示createBrowserHistory首次调用，进行初始化
  if (index == null) {
    index = 0;
  ...
  // 是否可以改变，如果有blocker事件则调用并阻塞改变，在blocker回调中调用retry可继续改变history
