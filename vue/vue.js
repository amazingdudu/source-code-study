...
(function(global, factory) {
	 ...
	 */
	//? val.toString === _toString考虑自定义toString?
	 ...
	 * toObject([{name:'tom'},{age:18}]) => {name:'tom',age:18}
	 */
	...
	var config = {
		 ...
		 * Platform-dependent.
		 * 检查是否必须使用attribute来绑定property，例如 value。取决于平台。
		 */
		mustUseProp: no,
	 ...
	 * 用于解析html标签，组件名称和属性路径的unicode字母。使用https://www.w3.org/TR/html53/semantics-scripting.html#potentialcustomelementname跳过\ u10000- \ uEFFFF，因为它冻结了PhantomJS
	 */
	...
	// 针对静态节点和插槽节点使用了优化的浅层克隆，因为它们可以在多个渲染中重复使用，
	...
	methodsToPatch.forEach(function(method) {
		...
		// 将methodsToPatch中所有的方法劫持后，添加到arrayMethods中
	...
	/*  */
	// 获取arrayMethods中自身的key(打过补丁的)；
	var arrayKeys = Object.getOwnPropertyNames(arrayMethods);

	/**
	 * In some cases we may want to disable observation inside a component's
	 * update computation.
	 * 在某些情况下，我们可能希望禁用组件更新计算中的observation。
	 */
	 ...
	 * 试图为值创建一个观察者实例，如果成功观察到，则返回新的观察者，如果值已经有，则返回现有的观察者。
	 */
	function observe(value, asRootData) {
		// 如果不是对象或者数组，或者是VNode实例
	...
	function defineReactive$$1(obj, key, val, customSetter, shallow) {
		...
		// 如果configurable标识该属性不能被修改，无法做响应式，直接返回
		...
		// 如果有两个参数，并且getter不存在，说明只传入obj和key，那么直接取val
		// ? !getter || setter 判断的原因
	...
	function set(target, key, val) {
		...
		// 如果是一个数组，那么使用splice方法实现更新
		if (Array.isArray(target) && isValidArrayIndex(key)) {
			target.length = Math.max(target.length, key);
			target.splice(key, 1, val);
			return val;
		}
		// 如果key已经是target的自身属性，那么直接赋值即可触发更新 此处是否用hasOwn(target,key)判断
	 ...
	 * 自定义合并策略的选项。策略选项分别接收在父实例和子实例上定义的该选项的值作为第一个和第二个参数，
	 * Vue 实例上下文被作为第三个参数传入。
	 */
	 ...
	 * el.propsData使用默认合并策略defaultStrat
	 */
	{
	...
	function mergeDataOrFn(parentVal, childVal, vm) {
		if (!vm) {
			...
			//   该函数返回两个函数的合并结果...此处无需检查parentVal是否为函数，
			//   因为它必须是传递先前合并的函数。Vue.extends()
	...
	strats.data = function(parentVal, childVal, vm) {
		// 只有在合并 data 选项，且是通过Vue.extends()处理子组件选项时，才会走 if 分支。处理正常的实例化选项 data 、 provide 时，都是走 else 分支。
		if (!vm) {
	 ...
	 * 当存在vm（创建实例）时，我们需要在构造函数选项，实例选项和父选项之间进行三向合并。
	 */
	...
	function normalizeProps(options, vm) {
		...
		if (Array.isArray(props)) {
			...
			for (var key in props) {
				val = props[key];
				name = camelize(key);
				// 如果val不是对象，则将val作为type https://cn.vuejs.org/v2/api/#props
	...
	function validateProp(key, propOptions, propsData, vm) {
		...
		if (booleanIndex > -1) {
			// propsData不存在key，并且prop中没有default属性
			if (absent && !hasOwn(prop, 'default')) {
				value = false;
			} else if (value === '' || value === hyphenate(key)) {
				// only cast empty string / same name to boolean if
				// boolean has higher priority
				// 如果布尔具有更高的优先级，则只将空字符串/同名转换为布尔值
	 ...
	 * 使用函数字符串名称来检查内置类型，
	 * 因为在不同的vms / iframe上运行时，简单的相等性检查将失败。
	 */
	function getType(fn) {
	...
	// 但是，当状态在重新绘制之前更改状态时（例如＃6813，外向过渡），它仍然存在一些细微问题。
	// 此外，在事件处理程序中使用（宏）任务会导致某些不可回避的怪异行为。
	// 因此，我们现在再次在各处使用微任务。这种权衡的主要缺点是，在某些情况下，微任务的优先级过高，
	// 并且在顺序事件之间触发（例如，＃4521） ，＃6690（有解决方法），甚至在冒泡同一事件之间（＃6566）。
	var timerFunc;
	...
	// nextTick行为利用了微任务队列，可以通过本机Promise.then或MutationObserver对其进行访问。 MutationObserver具有更广泛的支持，但是当在触摸事件处理程序中触发时，它在iOS> = 9.3.3的UIWebView中严重错误。 触发几次后，它将完全停止工作...因此，如果本地Promise可用，我们将使用它：
	...
	if (typeof Promise !== 'undefined' && isNative(Promise)) {
		...
		timerFunc = function() {
			...
			// 在有问题的UIWebViews中，Promise.then不会完全中断，但可能会陷入怪异的状态，在这种状态下，回调被推送到微任务队列中，但队列没有被刷新，直到浏览器需要执行其他一些工作，例如 处理一个计时器。 因此，我们可以通过添加一个空计时器来“强制”刷新微任务队列。
			if (isIOS) {
				setTimeout(noop);
			}
		};
	...
	// 对于纯HTML标记，可以完全跳过规范化，因为可以保证所生成的渲染函数返回Array <VNode>。
	// 在两种情况下需要额外的规范化：
	...
	// 当子级包含组件时-因为函数组件可能返回Array而不是单个根。 在这种情况下，只需要简单的规范化-如果任何子级是Array，我们将使用Array.prototype.concat来扁平化。 由于函数组件已经对其自己的子级进行了规范化，因此保证深度仅为1级。
	...
	// 当子级包含始终生成嵌套数组的构造时，例如 <template>，<slot>，v-for或用户为子级提供手写的渲染功能/ JSX时。 在这种情况下，需要完全规范化以适应所有可能类型的子代值。
	...
	function resolveInject(inject, vm) {
		// 如options定义了inject,是一个object
		if (inject) {
			var result = Object.create(null);
			var keys = hasSymbol ? Reflect.ownKeys(inject) : Object.keys(inject);

			//   遍历inject，依次取出所有的key值
			for (var i = 0; i < keys.length; i++) {
				var key = keys[i];
				...
				// 当前vue实例
				var source = vm;
				// 循环在父组件的_provided中查找provideKey对应的值
				while (source) {
				...
				// 如果没找到，查找自身default属性，如果不存在，输出警告
				if (!source) {
